 ...
 * }
    ...
    var pager=function(opts){
        ...
        var opts_default={
            ...
            loading:' <div class="x-table-zzc-img" title="1">'+
            ...
            render:function(vo,view){
                ...
                for(var i=0;i<vo.length;i++){
                    html+='<tr class="x-table-tr" data-index="'+i+'">'+self.options.tpl(vo[i])+'</tr>'
                }
                view.html(html);
            }
        };
    ...
    pager.prototype={
        ...
        ,init:function(){
            ...
            self.viewDom.addClass("x-table");
            //添加一个内部的div用来放置模板
            var outerHtml='<div class="x-table-outer">'+
                    //初始化过渡效果
                '<div class="x-table-zzc">'+opts.loading+'</div>'+
                '</div>';
            var viewOut=$(outerHtml);
            self.viewDom.after(viewOut);
            self.viewDom.appendTo(viewOut);
            self.viewOut=viewOut;
            var viewDom=self.viewDom;
            if(viewDom.find('tbody').length==0){
                viewDom.append('<tbody></tbody>');
            }
            self.templateView=viewDom.find('tbody');
            //注册单击事件
            self.templateView.on('click','.x-table-tr',function(e){
                var item=$(this);
                var itemData=self.data[self.options.dataName][item.attr('data-index')];
            ...
            if (self.options.pagePosition=="top") {
                viewOut.prepend('<div class="x-table-pagebar"></div>');
            }else{
                viewOut.append('<div class="x-table-pagebar"></div>');
            }
            self.pageBar=viewOut.find(".x-table-pagebar");
            fetch(response);
            function fetch(callback){
                //请求开始


                var mask_width=viewOut.outerWidth();
                var mask_height=viewOut.outerHeight();
                if(mask_height<100){
                    mask_height=100;
                }
                $(".x-table-zzc").css({
                ...
                $(".x-table-zzc-img").css({
                    "top":(mask_height-100)/2,
                    "left":(mask_width-30)/2
                });
                viewOut.find(".x-table-zzc").show();
                //请求开始
        ...
        render:function(vo){
            //请求结束
            this.viewOut.find(".x-table-zzc").hide();
            //请求结束-code ----end
            //vo :数据
            //view:用于填充模板的div对象
